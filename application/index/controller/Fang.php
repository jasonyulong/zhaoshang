...
class Fang extends Defa
{
    public function info(){
        if($this->request->isPost()) {
            ...
            $data = input('post.');
            db("Message")->insert(array("title"=>"厂房需求","time"=>time(),"member_id"=>$data["member_id"],"content"=>serialize($data)));
            $this->success("提交成功", url("index/index/index"),'',0);
        }else {
            $id = input('id', '', 'trim,intval');
            $info = db('Fang')->where(array('id' => $id))->find();
            if ($info) {
            ...
            $ran = db('Fang')->order("rand()")->limit(8)->select();
            ...
            db("Fang")->where(array('id' => $id))->setInc("look");
            return $this->fetch();
        }
    }
}
