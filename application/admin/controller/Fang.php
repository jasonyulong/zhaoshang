...
class Fang extends Admin
{
    function index(){
        $info=db('Fang')->order('id desc')->paginate(15,false,['query'=>request()->param()]);
    ...
    public function add(){
        if($this->request->isPost()){
            $data=input('post.');
            $mod=new \app\common\model\Fang();
            $result=$mod->add($data,$this->request->file('pic'));
            if($result!==false){
                $this->success("成功", url("admin/fang/index"),'',0);
            }else{
